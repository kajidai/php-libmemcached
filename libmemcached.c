...
static int _php_libmemcached_error(memcached_st *, memcached_return TSRMLS_DC);
...
PHP_MINIT_FUNCTION(libmemcached)
{
    ...
    REGISTER_LONG_CONSTANT("MEMCACHED_BEHAVIOR_KETAMA_WEIGHTED", MEMCACHED_BEHAVIOR_KETAMA_WEIGHTED, CONST_CS | CONST_PERSISTENT);
    REGISTER_LONG_CONSTANT("MEMCACHED_BEHAVIOR_KETAMA_HASH", MEMCACHED_BEHAVIOR_KETAMA_HASH, CONST_CS | CONST_PERSISTENT);
    REGISTER_LONG_CONSTANT("MEMCACHED_BEHAVIOR_BINARY_PROTOCOL", MEMCACHED_BEHAVIOR_BINARY_PROTOCOL, CONST_CS | CONST_PERSISTENT);
    ...
    REGISTER_LONG_CONSTANT("MEMCACHED_BEHAVIOR_SERVER_FAILURE_LIMIT", MEMCACHED_BEHAVIOR_SERVER_FAILURE_LIMIT, CONST_CS | CONST_PERSISTENT);
...
static int _php_libmemcached_error(memcached_st *res_memc, memcached_return error TSRMLS_DC) {
    int status = 0;
    ...
    switch (error) {
        ...
        default:
            if (res_memc != NULL) {
                int i;
                for (i=0; i<res_memc->number_of_hosts; i++) {
                    host = res_memc->hosts[i];
                    if (host.server_failure_counter > 0) {
                        php_error_docref(NULL TSRMLS_CC, E_WARNING, "hostname=%s, cached_errno=%d:\"%s\", server_failure_counter=%d", host.hostname, host.cached_errno, strerror(host.cached_errno), host.server_failure_counter);
                        status = -1;
                    }
                }
            }
            if (status >= 0) {
                php_error_docref(NULL TSRMLS_CC, E_WARNING, "%s", memcached_strerror(NULL, error));
            }
            status = -1;
            break;
    }
...
PHP_FUNCTION(memcached_get)
{
    ...
    if (_php_libmemcached_error(res_memc, rc) < 0) {
        RETURN_FALSE
    }
...
PHP_FUNCTION(memcached_replace)
{
    ...
    val = _get_value_from_zval(&buf, var, &len, &flags TSRMLS_CC);
    memcached_return rc;
    rc = memcached_replace(res_memc, key, key_len, val, len, (time_t)expiration, (uint16_t)flags);
    smart_str_free(&buf);
    efree(val);
    if (rc != MEMCACHED_SUCCESS) {
        RETURN_FALSE;
...
PHP_FUNCTION(memcached_replace_by_key)
{
    ...
    val = _get_value_from_zval(&buf, var, &len, &flags TSRMLS_CC);
    memcached_return rc;
    rc = memcached_replace_by_key(res_memc, master_key, master_key_len, key, key_len, val, len, (time_t)expiration, (uint16_t)flags);
    smart_str_free(&buf);
    efree(val);
    if (rc != MEMCACHED_SUCCESS) {
        RETURN_FALSE;
